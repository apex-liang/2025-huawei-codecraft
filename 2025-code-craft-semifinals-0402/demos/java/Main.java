...
public class Main {
    ...
    private static void readAction() {
        ...
        processCurrentReadRequest(nRead, nBusy, reqs);
    }

    private static void processCurrentReadRequest(int nRead, int nBusy, int[] reqs) {
        int objectId;
        if (currentRequest == 0) {
            for (int i = 1; i <= N; i++) {
                System.out.println("#\n#");

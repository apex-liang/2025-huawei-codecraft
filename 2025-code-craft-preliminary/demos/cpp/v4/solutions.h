...
inline set<int> free_blocks[MAX_DISK_NUM][MAX_LABEL_NUM+2];//多分配几个
...
inline vector<pair<int,pair<int,int>>> disk_label[MAX_DISK_NUM];//disk存储的LABEL
inline unordered_map<int,int> range_begin[MAX_LABEL_NUM];//存储标签M的disk_id和block_id
inline vector<int> label_disk[MAX_LABEL_NUM];//LABEL存储的disk_id,优先值
inline vector<int> label_disk_2[MAX_LABEL_NUM];//备份2对应
...
inline unordered_map<int,int> replica_3_label;
inline pair<long long , int> hot_rank[MAX_LABEL_NUM][MAX_LABEL_NUM];
...
inline int disk_label_new[11][3];
inline bool disk_label_new_st[17];
inline vector<int> label_disk_new[17];

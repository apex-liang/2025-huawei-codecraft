...
void read_action()
{
    ...
    if(tt-hh>1100) sort(q+tt-700,q+tt,cmp);//只排序当前时间近的前700个
	...
	for(int i=1;i<=N;i++){
        ...
        if( ((disk_point[i]> max_have_data_pos[i][disk_point_range[time_stemp][i].second-1]))||( disk_point[i] > disk_point_range[time_stemp][i].first.second )){

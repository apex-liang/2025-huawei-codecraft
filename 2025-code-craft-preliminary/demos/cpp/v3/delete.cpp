...
void do_object_delete(const int* object_unit, int* disk_unit, int size,int object_id,int disk_id,int replica_id)
{
    for (int i = 1; i <= size; i++) {
        ...
        if(object_unit[i]>disk_begin[disk_id])
            free_space[disk_id].insert(object_unit[i]);
        else{
            for(int j=0;j<disk_label[disk_id].size();j++)
            {
                int begin_tmp=disk_label[disk_id][j].second.first;
                int end_tmp=disk_label[disk_id][j].second.second;
                if((object_unit[i]<=end_tmp)&&(object_unit[i]>=begin_tmp))
                    free_blocks[disk_id][j+1].insert(object_unit[i]);
            }
        }
